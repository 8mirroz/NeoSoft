...
func update(delta: float) -> void:
	...
	if current_timer_sec <= 0.0:
		...
		GameEventBus.emit_signal("fever_ended") # Информируем шину об истечении комбо
	else:
...
func open_window() -> void:
	...
	GameEventBus.emit_signal("fever_meter_changed", current_timer_sec / combo_duration_sec * 100.0)
