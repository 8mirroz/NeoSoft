# Установленные навыки и плагины — Genesis v1

Этот документ фиксирует плагины, навыки (skills) и операционные правила разработки, установленные и активные в текущем воркспейсе игры **Neo Soft Frost**.

---

## 1. Активные навыки (Active Skills)

В соответствии с результатами сканирования и анализа требований MVP релиза, в воркспейсе активированы следующие специализированные расширения Antigravity:
...
1.  **RULE-001: Source-of-Truth Hierarchy**:
    Машиночитаемые файлы и директория `genesis/v1/` имеют безусловный приоритет над кодом и нарративными доками.
...
3.  **RULE-003: Визуальные гейты**:
    Жесткие диапазоны длительности анимаций (Select: 0.18-0.35s, Swap: 0.22-0.35s) и лимиты читаемости фишек на мелких экранах.
...
5.  **RULE-005: Архитектурные запреты**:
    Шина событий `EventBus` как единственный мост связи между логическим ядром и UI. Запрет прямой зависимости логики от графики.
6.  **RULE-006: VFX Quality Gates**:
    Запрет пересветов на светлом фоне, плавностьSPRING-анимаций (Squash & Stretch).
